 ...
 */
@RestController
@RequestMapping("/v1/parametros/tipos")
public class TipoController {
	 ...
	 */
	@ApiOperation(value = "operación de servicio que consulta el listado de los tipos", notes = "La operación retorna todas los tipos registrados en la base de datos")
	...
	@ApiOperation(value = "operación de servicio que consulta el listado de los tipos por clase", notes = "La operación retorna los tipos por clase registrados en la base de datos")
	@ApiResponses(value = { @ApiResponse(code = 500, message = ParametrosMensajes.ERROR_SERVER),
			@ApiResponse(code = 404, message = ParametrosMensajes.ERROR_NO_DATA),
			@ApiResponse(code = 200, message = ParametrosMensajes.RESPUESTA_CORRECTA) })
	@GetMapping("/{clase}")
	public ResponseEntity<List<SnrTipoDTO>> listarPorTipo(@PathVariable("clase") String clase) throws SiprenException {
		List<SnrTipoDTO> lista = service.listarPorClase(clase).stream().map(n -> this.modelMapper.map(n, SnrTipoDTO.class))
				.collect(Collectors.toList());
		if (lista != null && lista.isEmpty())
			throw new ModeloNotFoundException(ParametrosMensajes.ERROR_NO_DATA);
	 ...
	 * @throws SiprenException
	 */
	@ApiOperation(value = "operación de servicio que registra un nuevo tipo", notes = "La operación registra el parametro en base de datos y retorna el registro")
	@ApiResponses(value = { @ApiResponse(code = 500, message = ParametrosMensajes.ERROR_SERVER),
	 ...
	 * @throws SiprenException
	 */
	@ApiOperation(value = "operación de servicio que actualiza un tipo", notes = "La operación actualiza el parametro en base de datos y retorna el registro")
	@ApiResponses(value = { @ApiResponse(code = 500, message = ParametrosMensajes.ERROR_SERVER),
			@ApiResponse(code = 200, message = ParametrosMensajes.RESPUESTA_CORRECTA) })
	@PutMapping
	public ResponseEntity<SnrTipoDTO> modificar(@Valid @RequestBody SnrTipoDTO dto)
			throws SiprenException {
